...
if(isset($_POST['change']))
{
   $email=$_POST['email'];
    ...
    $authenticationkey=md5($_POST['confirmauthenticationkey']);
$query=mysqli_query($con,"SELECT * FROM users WHERE email='$email' and contactno='$contact'");
$num=mysqli_fetch_array($query);
if($num>0)
{
$extra="forgot-password.php";
mysqli_query($con,"update users set password='$password', authenticationkey='$authenticationkey' WHERE email='$email' and contactno='$contact' ");
$host=$_SERVER['HTTP_HOST'];
$uri=rtrim(dirname($_SERVER['PHP_SELF']),'/\\');
header("location:http://$host$uri/$extra");
$_SESSION['errmsg']="Password & Authentication Key Changed Successfully";
exit();
}
else
{
...
<div class="form-group">
	    	<label class="info-title" for="confirmauthenticationkey">Confirm Authentication Key. <span>*</span></label>
	    	<input type="password" class="form-control unicase-form-control text-input" id="confirmauthenticationkey" name="confirmauthenticationkey" required >
	  	</div>
